...
build_os() {
  ...
  for file in $(find nedclib | grep "\.cpp$"); do
    ${CC_PREFIX}g++ $CFLAGS -flto -DNEDCLIB_EXPORTS -c -g -fPIC -o build/$1/obj/$(basename $file).o -Wall -O2 -std=c++0x -I nedclib -I nedclib/rawbmp $file
  done
  ...
  for prog in nedcenc raw2bmp nedcmake nevpk; do
    ${CC_PREFIX}g++ $CFLAGS -flto -O2 -Wall -o build/$1/${prog}${EXE_EXTENSION} $prog/*.cpp build/$1/nedclib.a
  done
}
